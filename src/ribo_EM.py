...
def sparse_choosecols(spmat):
	...
	rowcsvals = (fres[2] -  offset_csmax[fres[0]].reshape(len(fres[0])))
	#everything greater than the float is on the menu
	rowchoicefloats = np.random.random([n_reads])[fres[0]]
	...
	assert spmat_t.shape ==(n_trs,n_reads)
