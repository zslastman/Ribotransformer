...
# nblosarray[0] - nblosarray[2]
# txtplot(nblosarray)
# poislosarray = np.array([poisloss(ten([i]).log(),ten([10])) for i in range(6,15)])
# txtplot(poislosarray)

# nblosarray1 = np.array([nbregcriterion(ten([i]).log(),ten([10]),psi=1) for i in range(6,15)])
# nblosarray2 = np.array([nbregcriterion(ten([i]).log(),ten([10]),psi=1.05) for i in range(6,15)])
# nblosarray3 = np.array([nbregcriterion(ten([i]).log(),ten([10]),psi=100) for i in range(6,15)])
# txtplot(np.concatenate([nblosarray1,nblosarray2]))

# txtplot(np.concatenate([nblosarray3-poislosarray]))


# txtplot(nblosarray2 - nblosarray1)
