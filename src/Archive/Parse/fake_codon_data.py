...
if True:
    ...
    starteffect = np.concatenate(
        [1.5**(-np.linspace(0, 10, 10)), np.array([0] * (512 - 10))])
    assert starteffect.shape == (n_len,)
    stopeffect = np.concatenate(
        [np.array([0] * (512 - 10)), 1.5**(-np.linspace(0, 10, 10))])
    assert stopeffect.shape == (n_len,)
    #
    ...
    if poseffects:
        fakesignal = fakesignal * \
            np.exp(starteffect.reshape([n_len, 1])) * \
            np.exp(stopeffect.reshape([n_len, 1]))
